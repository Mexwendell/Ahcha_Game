...
class NLReloadComponentsAllObjs(bpy.types.Operator):
    bl_idname = "bge_netlogic.reload_components_all_objs"
    bl_label = "Reload Components in All Objects"
    bl_options = {'REGISTER', 'UNDO'}
    bl_description = "Reload all components applied to all objects"

    def execute(self, context):
        # Salvar o objeto ativo atual
        original_active_obj = context.view_layer.objects.active

        for obj in bpy.data.objects:
            # Garantir que o objeto esteja no ViewLayer ativo
            if obj.name in context.view_layer.objects:
                if hasattr(obj, "game") and hasattr(obj.game, "components"):
                    components = obj.game.components
                    if components:
                        # Tornar o objeto ativo temporariamente para chamar o operador
                        context.view_layer.objects.active = obj
                        for i in range(len(components)):
                            try:
                                bpy.ops.logic.python_component_reload(index=i)
                            except RuntimeError as e:
                                self.report({"WARNING"}, f"Erro ao recarregar componente em {obj.name}: {e}")

        # Restaurar o objeto ativo original
        context.view_layer.objects.active = original_active_obj

        self.report({"INFO"}, "Todos os componentes foram recarregados.")
        return {'FINISHED'}
